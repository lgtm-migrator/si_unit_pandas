...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive si_unit_pandas/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive si_unit_pandas/
